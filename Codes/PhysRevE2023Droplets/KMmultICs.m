function [x] = KMmultICs(x0, T_max, gamma)
...
for t = dt:dt:4 % Discretization of intermediate projectile motion
    ...
    if eta < Psi + R/(g/(2*pi*f)^2) % Just past impact
        ...
        for i = 1:10000
            ...
            Psi = A*sin(pi/2 + 2*pi*t_impact)*sin(pi/2 + 2*pi*x0) + gamma*sin(pi/2 + 2*pi*(2*t_impact-varphi));
    ...
    dPsidt = -A*2*pi*sin(2*pi*t_impact)*sin(pi/2 + 2*pi*x0) - 4*pi*gamma*sin(2*pi*(2*t_impact-varphi));
