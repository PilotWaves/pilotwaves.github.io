...
%function taux = KM_Timeseries

%%% Dimensional parameters in mm, kg, s

g = 9810;
gammaF = 4.29*g; % dim bath forcing
%gamma = 1.0054*gammaF; T_max = 2500;
gamma = 1.047*gammaF; T_max = 1000000;
...
taux(count,:) = [T, x, T - Tprev, vx*lam_f/Tf/10, vy*(g/(2*pi*f)^2)/Tf/10];
...
for t = dt:dt:4  % Discretization of intermediate projectile motion
    ...
    if eta > Psi + R/(g/(2*pi*f)^2)
        ...
        for i = 1:10000
            ...
            count = count+1;
            taux(count,:) = [T, x, T - Tprev, vx*lam_f/Tf/10, vy*(g/(2*pi*f)^2)/Tf/10]; %velocity in cm
            Tprev = T;
...
% axis([0 T_max min(taux(:,2)) max(taux(:,2))])
% 
% toc
